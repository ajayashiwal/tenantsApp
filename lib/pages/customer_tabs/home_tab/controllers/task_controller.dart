...
class CloseTaskPageController extends GetxController implements ApiInterface{
  ...
  void onSuccess(data, code) {
    ...
    Duration date6=date2.difference(date4);
     parsentage2= (100 -double.parse(((date6.inDays/date2.day)*100).toStringAsFixed(2))).toStringAsFixed(2);
    parsentage4= 1 - double.parse(((date6.inDays/date2.day)).toStringAsFixed(2));
    ...
    parsentage22= (double.parse(((dateDiff.inDays/dateDifference)*100).round().toStringAsFixed(0))).toStringAsFixed(0);
    ...
    if(projectTaskDetailModel.data!.lawyers!=null) {
      if(projectTaskDetailModel.data!.lawyers!.isNotEmpty) {
        ...
        if(projectTaskDetailModel.data!.taskId==1 || projectTaskDetailModel.data!.taskId==3) {
          for (int i = 0; i < projectTaskDetailModel.data!.lawyers!.length; i++) {
            int d = projectTaskDetailModel.data!.lawyers![i].chooseByTenantCount;
          ...
          if(projectTaskDetailModel.data!.lawyers!.length>secondMaxValuePosition){
            projectTaskDetailModel.data!.lawyers![secondMaxValuePosition].isSelected = true;
          }
        }
        projectTaskDetailModel.data!.lawyers![firstMaxValuePosition].isSelected = true;

      }
    }
    if(projectTaskDetailModel.data!.contractors!=null) {
      if(projectTaskDetailModel.data!.contractors!.isNotEmpty) {
        for (int i = 0; i < projectTaskDetailModel.data!.contractors!.length; i++) {
          int d = projectTaskDetailModel.data!.contractors![i].chooseByTenantCount;
          if(firstMaxValue<d){
            firstMaxValue = d;
            firstMaxValuePosition = i;
          }
        ...
        if(projectTaskDetailModel.data!.taskId==2 || projectTaskDetailModel.data!.taskId==4) {
          for (int i = 0; i < projectTaskDetailModel.data!.contractors!.length; i++) {
            int d = projectTaskDetailModel.data!.contractors![i].chooseByTenantCount;
            if (secondMaxValue < d && d < firstMaxValue) {
              secondMaxValue = d;
          ...
          if(projectTaskDetailModel.data!.contractors!.length>secondMaxValuePosition){
            projectTaskDetailModel.data!.contractors![secondMaxValuePosition].isSelected = true;
          }
        }
        projectTaskDetailModel.data!.contractors![firstMaxValuePosition].isSelected = true;
